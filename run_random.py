...
def predict(args, logger, model, task_model, predict_data):
    ...
    enc_routes0 = torch.nn.functional.one_hot(torch.rand(model.config.encoder_layers, n_expert).argmax(dim=1),n_expert).float()
    # enc_routes0 = (1 - enc_routes0) / (n_expert - 1)
    dec_routes0 = torch.nn.functional.one_hot(torch.rand(model.config.decoder_layers, n_expert).argmax(dim=1),n_expert).float()
    # dec_routes0 = (1 - dec_routes0) / (n_expert - 1)
...
def train(args, logger, model, task_model, train_data, dev_data, dev_data2, optimizer, scheduler):
    ...
    for epoch in range(int(args.num_train_epochs)):
        for batch in tqdm(train_data.dataloader, desc="Epoch {}".format(epoch)):
            ...
            n_expert = model.config.router_block_num
            enc_routes = torch.nn.functional.one_hot(torch.rand(bsz * model.config.encoder_layers, n_expert).argmax(dim=1),n_expert).float()
            # enc_routes = (1 - enc_routes) / (n_expert - 1)
            enc_routes = enc_routes.reshape(model.config.encoder_layers, bsz, -1)
            dec_routes = torch.nn.functional.one_hot(torch.rand(bsz * model.config.decoder_layers, n_expert).argmax(dim=1),n_expert).float()
            # dec_routes = (1 - dec_routes) / (n_expert - 1)
            dec_routes = dec_routes.reshape(model.config.decoder_layers, bsz, -1)

            if torch.cuda.is_available():
                enc_routes = enc_routes.to(torch.device("cuda"))
                dec_routes = dec_routes.to(torch.device("cuda"))
